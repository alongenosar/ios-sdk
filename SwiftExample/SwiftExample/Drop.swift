...
@objc public enum DropStateObjC:Int {
    case ObjcInfo
    case ObjcSuccess
    case ObjcWarning
    case ObjcError
...
@objc public final class Drop: UIView {
...
extension Drop {
    ...
    @objc public class func downObjc(_ status: String,state: DropStateObjC, duration: Double = 4.0, action: DropAction? = nil) {
       
        var s: DropState?
        switch state {
            case DropStateObjC.ObjcError:
                s = .error
            break
            case DropStateObjC.ObjcInfo:
                s = .info
            break
            case DropStateObjC.ObjcWarning:
                s = .warning
            break
            case DropStateObjC.ObjcSuccess:
                s = .success
            break
        }
        show(status, state: s!, duration: duration, action: action)
    }
